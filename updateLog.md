...
### 4.3.1.170221
- PVW를 사용하지 않는 옵션에서 나타나는 <kbd>+</kbd>/<kbd>-</kbd>(<kbd>[</kbd>/<kbd>]</kbd>) 키 오작동을 수정했습니다.

### 4.3.0.170118
- PVW를 사용하지 않는 옵션을 추가했습니다. 이 옵션을 켜면 PVW를 거치지 않고 즉시 내보냅니다.

## 4.2

### 4.2.0.160914
- 텍스트 라이브 송출 기능을 추가했습니다.
- 텍스트 상하 정렬 위치를 변경했습니다.
- 버튼 클릭 후 키보드로 조작할 때 오작동하지 않도록 보강했습니다.
- 메인 창을 새로 열 때마다 새로운 세션 번호가 부여되어, 중복 실행이 가능합니다. 이 번호는 컴퓨터 시간에 따라 달라지므로 시간을 변경하지 않는 이상 중복되지 않습니다.

## 4.1

### 4.1.0.160704
- 텍스트 정렬(위치 이동) 설정을 추가했습니다.
- 중간 레이아웃이 변경되었습니다.
