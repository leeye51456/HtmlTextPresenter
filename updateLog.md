...
- 3.0-beta1 버전에서 송출 내용을 지울 때 송출 창에는 반영되지 않는 문제가 확인되어 올바르게 작동하도록 수정했습니다.
- PGM/PVW 뷰가 flexible box로 대체되며, 송출 창의 화면비를 반영하지 않도록 수정되었습니다.
- CSS 인코딩이 맞지 않는 문제를 수정했습니다.
- PVW/PGM 뷰에 각각 녹색/적색 테두리가 표시됩니다. PGM이 비어 있는 경우 회색 테두리가 표시됩니다.
- CUT, CLEAR 내부 동작을 수정했습니다.
- 3.0-beta4 버전에서 PVW가 1페이지인 상태에서 앞 페이지를 PVW에 올리는 동작(`0`-`Enter`, `Num-`, `[`)을 하면 동작에는 문제가 없지만 오류가 발생하던 것을 올바르게 작동하도록 수정했습니다.
- 방향키 `↑`, `↓` 키를 누르면 페이지 리스트 영역이 스크롤됩니다.
- 송출 창 내용 표시 수단을 table에서 flexible box로 변경하였습니다.
...
- PVW/PGM 업데이트 시 버그 수정
- 텍스트 파일을 불러오지 않고 송출 창을 열고 닫을 때 생기는 버그를 수정했습니다.
